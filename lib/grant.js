...
function usePasswordGrant (done) {
  ...
  var uname = this.req.body.username,
    ...
    devId = this.req.body.device_id,
    devType = this.req.body.device_type;
  ...
  if(!devId || !devType) {
    return done(error('invalid_client', 'Missing parameters. "device_type" and "device_id" are required'));
  }

  var self = this;
  return this.model.getUser(uname, pword, devType, devId, function (err, user) {
    if (err) return done(error('server_error', false, err));
    if (!user) {
